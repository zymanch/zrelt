...
class yiiDebugRest extends yiiDebugClass
{
    public static function getInfo($data = array(), $config = null)
    {
        parent::getInfo($data);
        $restLogs = Core_Module_Client_Client::getQueryParams();
        $restTimes = Core_Registry::logger()->getPoints('rest');
        $result = array(
            'title' => 'REST: '.sizeof($restLogs),
            'panelTitle' => 'REST Log',
        );
        $items = array();
        foreach ($restLogs as $index => $item) {
            $items[] = array(
                print_r($item[0], 1)."\n".$item[1]['debug_url'],
                '',
                'rest',
                mktime(0,0,0,1,1,1971) + $restTimes[$index]['time']
            );
        }
        if (count($items) > 0) $result['content'] = yiiDebugTrace::render($items);
